...
ComputadorasEscritorio::ComputadorasEscritorio(int id, string const &nombre, int criticidad, bool enUso, int incidenciasActivas, int tiempoInactivo) : Equipo(id, nombre, incidenciasActivas, tiempoInactivo, criticidad, enUso) {
    ...
    //Si la computadora de escritorio no está en uso, ajustar el tiempo inactivo a 0 y bajar 5 puntos de criticidad (en este caso si se puede tener una criticidad menor a 7, ya que la computadora de escritorio no está en uso)
    if (!enUso) {
